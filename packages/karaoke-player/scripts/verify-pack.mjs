...
try {
  ...
  for (const bundle of ["dist/index.js", "dist/index.cjs"]) {
    const source = readFileSync(new URL(bundle, moduleRoot), "utf8");
    if (!source.startsWith("'use client';")) {
      throw new Error(`${bundle} is missing its Next.js client boundary`);
    }
  }
  const esm = await import(new URL("dist/index.js", moduleRoot));
  if (typeof esm.KaraokePlayer !== "function")
    throw new Error("ESM KaraokePlayer export missing");
  ...
  const commonJs = require(
    fileURLToPath(new URL("dist/index.cjs", moduleRoot)),
  );
