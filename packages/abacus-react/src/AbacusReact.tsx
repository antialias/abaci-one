...
export interface AbacusConfig {
  // Basic configuration
  /** Controlled value - when provided, component syncs to this value */
  value?: number | bigint;
  /** Uncontrolled default value - used as initial value when `value` is undefined */
...
export function useAbacusState(
  ...
  const value = useMemo(() => {
    return columnStates.reduce((total, columnState, index) => {
      const placeValue = 10 ** (columnStates.length - index - 1);
      const columnValue =
...
// Supports both controlled mode (value prop) and uncontrolled mode (defaultValue prop)
export function useAbacusPlaceStates(
  controlledValue: number | bigint | undefined,
  maxPlaceValue: ValidPlaceValues = 4,
  defaultValue: number | bigint = 0,
) {
  // Determine if we're in controlled mode
  const isControlled = controlledValue !== undefined;
  // For initialization, use controlled value if provided, otherwise default
  ...
  const value = useMemo(() => {
    ...
    if (useBigInt) {
      ...
      let total = 0;
      placeStates.forEach((state) => {
        const placeValueNum = 10 ** state.placeValue;
  ...
  // Only runs in CONTROLLED mode (when value prop is provided)
  // In uncontrolled mode (only defaultValue), internal state is independent
  React.useEffect(() => {
    // Skip if uncontrolled - let internal state be independent
  ...
  }, [controlledValue, initializeFromValue, value, isControlled]);
...
function calculateValueFromColumnStates(
  ...
  columnStates.forEach((columnState, index) => {
    const placeValue = 10 ** (totalColumns - 1 - index);
    const columnValue =
      (columnState.heavenActive ? 5 : 0) + columnState.earthActive;
...
function calculateValueFromPlaceStates(
  ...
  if (useBigInt) {
    ...
    for (const [placeValue, placeState] of placeStates) {
      const digitValue =
        (placeState.heavenActive ? 5 : 0) + placeState.earthActive;
      value += digitValue * 10 ** placeValue;
    }
    return value;
  }
}
...
export const AbacusReact: React.FC<AbacusConfig> = ({
  ...
  // Effective value for display calculations (use defaultValue if value not provided)
  const displayValue = value ?? defaultValue;

  // Calculate effective columns first, without depending on columnStates
  const effectiveColumns = useMemo(() => {
    if (columns === "auto") {
      const minColumns = Math.max(1, displayValue.toString().length);
      return showEmptyColumns ? minColumns : minColumns;
    }
    return columns;
  }, [columns, displayValue, showEmptyColumns]);

  // Detect system theme for automatic numeral color adjustment
  const systemTheme = useSystemTheme();

  // Switch to place-value architecture!
  // Pass value (may be undefined for uncontrolled) and defaultValue separately
  ...
  const {
    ...
    getPlaceState,
    setPlaceState,
  } = useAbacusPlaceStates(value, maxPlaceValue, defaultValue);
