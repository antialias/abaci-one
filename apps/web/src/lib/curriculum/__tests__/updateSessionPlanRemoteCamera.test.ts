...
const { updateSessionPlanRemoteCamera, getSessionPlan } = await import('../session-planner')
...
describe('updateSessionPlanRemoteCamera', () => {
  ...
  it('should set remote camera session ID on a session plan', async () => {
    ...
    const updatedPlan = await updateSessionPlanRemoteCamera(testPlanId, remoteCameraSessionId)
  ...
  it('should not affect other session plan fields', async () => {
    ...
    expect(updatedPlan?.targetDurationMinutes).toBe(initialPlan?.targetDurationMinutes)
    expect(updatedPlan?.status).toBe(initialPlan?.status)
    expect(updatedPlan?.estimatedProblemCount).toBe(initialPlan?.estimatedProblemCount)
  })
})
