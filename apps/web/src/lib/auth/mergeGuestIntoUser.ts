...
export async function mergeGuestIntoUser(
  ...
  // Design snapshots (#22): an unshared design is owner-gated, so a guest's
  // ?design= links (printed on their THH job cards) must keep resolving after
  // sign-in — and this one line is also why sharing (#24) needed no merge code
  // of its own: the share flag rides this very row. The rows' content hashes
  // stay scoped to the OLD id — that only means a future identical submit
  // mints a fresh row instead of deduping, which is fine.
