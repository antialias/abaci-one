...
export const PATCH = withAuth(async (request, { params }) => {
  ...
  try {
    ...
    const update: { enabled?: boolean; config?: string | null; description?: string | null; allowedRoles?: string | null } = {}
    ...
    if ('allowedRoles' in body) {
      if (body.allowedRoles === null) {
        ...
        update.allowedRoles = body.allowedRoles.length > 0 ? JSON.stringify(body.allowedRoles) : null
      } else {
        return NextResponse.json(
