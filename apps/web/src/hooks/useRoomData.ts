...
export interface AutoLeaveInfo {
  roomIds: string[]
  message: string
}
 ...
 * Create a new room
 */
async function createRoomApi(params: CreateRoomParams): Promise<CreateRoomResult> {
...
export function useCreateRoom() {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      // Log auto-leave info if any rooms were left
      if (result.autoLeave && result.autoLeave.roomIds.length > 0) {
        console.log(
          `[useCreateRoom] Auto-left ${result.autoLeave.roomIds.length} room(s):`,
          result.autoLeave.roomIds
        )
      }
    },
  })
