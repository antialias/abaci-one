...
export function useRealtimeVoice(options?: UseRealtimeVoiceOptions): UseRealtimeVoiceReturn {
  ...
  const setNarrationPlaying = useCallback((playing: boolean) => {
    ...
    if (audioElRef.current) {
      ...
      console.log(
        '[exploration] agent audio %s (narration %s)',
        playing ? 'muted' : 'unmuted',
  ...
  const updateSession = useCallback(
    (dc: RTCDataChannel, modeId?: ModeId, action?: string) => {
  ...
  const enterMode = useCallback(
    (dc: RTCDataChannel, newMode: ModeId, savePrevious = true, action?: string) => {
      console.log(
        '[mode] enterMode: %s → %s (savePrevious=%s, previous=%s)',
  ...
  const dial = useCallback(
    async (number: number, options?: DialOptions) => {
      ...
      const previousScenario =
        priorCalls.length > 0 ? priorCalls[priorCalls.length - 1].scenario : null
      ...
      try {
        ...
        setError(
          isFetchFailure ? `Couldn't reach ${number}. Check your internet and try again!` : msg
        )
      ...
      try {
          ...
          try {
            ...
            if (msg.type === 'error') {
              ...
              if (
                ...
                errCode === 'item_truncation_failed' ||
            ...
            if (msg.type === 'response.created' || msg.type === 'response.done') {
              console.log(
                '[exploration] %s — response_id: %s, pending: %s, narrationPlaying: %s',
                msg.type,
                msg.response?.id,
                pendingExplorationRef.current
                  ? JSON.stringify(pendingExplorationRef.current)
                  : 'none',
            ...
            if (msg.type === 'response.created' && narrationPlayingRef.current) {
              if (!isExplorationActiveRef?.current) {
                ...
                console.log(
                  '[exploration] cancelling response %s — narration is playing',
                  msg.response?.id
                )
            ...
            if (msg.type === 'response.done') {
              ...
              console.log(
                '[exploration] response.done — pending:',
              ...
              if (pending) {
                ...
                if (pending.type === 'start') {
                  ...
                  const executeStart = () => {
                    console.log(
                      '[exploration] executing deferred start_exploration:',
                      pending.constantId
                    )
                  ...
                  console.log(
                    '[exploration] waiting for %dms sustained silence before executing start',
                  ...
                  const waitForSilence = () => {
                    if (Date.now() - startedWaiting > 3000) {
                      console.log(
                        '[exploration] silence wait timed out after 3s — executing anyway'
                      )
                      executeStart()
                      return
                    }
                    ...
                    console.log(
                      '[exploration] sustained silence confirmed — executing start after %dms',
                  ...
                  if (audioItem?.id) {
                    ...
                    console.log(
                      '[exploration] truncating audio item %s at %dms — cutting off agent speech',
                      audioItem.id,
                      audioEndMs
                    )
                    dc.send(
            ...
            if (msg.type === 'response.function_call_arguments.done') {
              console.log(
                '[conference] tool call: %s, args: %s, speaker: %s, conference: %s',
                msg.name,
                msg.arguments?.slice(0, 80),
                currentSpeakerRef.current,
                conferenceNumbersRef.current.join(',')
              )
            }
            ...
            if (activeGameIdRef.current) {
              ...
              console.log(
                '[mode] tool call %s — activeGame: %s, hasOnToolCall: %s, matchesSessionTool: %s',
                msg.name,
              ...
              dc.send(
                JSON.stringify({
                  ...
                  item: {
                    ...
                    output: JSON.stringify({
                      success: true,
                      message: `Transferring to ${targetNumber}`,
                    }),
                  },
                })
              )
            ...
            if (msg.name === 'add_to_call') {
                ...
                dc.send(
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        success: false,
                        error: 'Invalid target_numbers array',
                      }),
                    },
                  })
                )
                ...
                dc.send(
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        success: true,
                        message: 'All those numbers are already on the call!',
                      }),
                    },
                  })
                )
              ...
              console.log(
                '[conference] add_to_call: adding %s, all on call: %s',
                joinedNames,
                updated.join(', ')
              )
              ...
              dc.send(
                JSON.stringify({
                  ...
                  item: {
                    ...
                    output: JSON.stringify({
                      success: true,
                      message: `${joinedNames} joined the call!`,
                    }),
                  },
                })
              )
              ...
              const existingNumbers = conferenceNumbersRef.current.filter(
                (n) => !newNumbers.includes(n)
              )
              const introText =
                newNumbers.length === 1
            ...
            if (msg.name === 'switch_speaker') {
                ...
                dc.send(
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        success: false,
                        error: `${targetNumber} is not on the call`,
                      }),
                    },
                  })
                )
              ...
              console.log(
                '[conference] switch_speaker: %d → %d (pending audio onset)',
                prevSpeaker,
                targetNumber
              )
            ...
            if (msg.name === 'start_exploration') {
              ...
              console.log(
                '[exploration] start_exploration tool call — deferring until response.done, constantId:',
                constantId
              )
            ...
            if (msg.name === 'resume_exploration') {
              ...
              console.log(
                '[exploration] resume_exploration tool call — cancelling speech + deferring until response.done'
              )
                ...
                dc.send(
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        success: true,
                        message: `Jumped to segment ${segNum} (paused)`,
                      }),
                    },
                  })
                )
                ...
                dc.send(
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        success: true,
                        message: `Looking at ${center} (range ${range})`,
                      }),
                    },
                  })
                )
                ...
                dc.send(
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        success: false,
                        error: 'No active scenario to evolve',
                      }),
                    },
                  })
                )
                        ...
                        JSON.stringify({
                          ...
                          item: {
                            ...
                            output: JSON.stringify({
                              success: true,
                              message:
                                'The story is flowing well — no twist needed right now. Keep going with the current thread.',
                            }),
                          },
                        })
                      )
                      ...
                      JSON.stringify({
                        ...
                        item: {
                          ...
                          output: JSON.stringify({
                            ...
                            suggestion,
                            message:
                              'Weave this development into the conversation naturally. Do not dump it all at once — let it unfold.',
                          }),
                        },
                      })
                    )
                      ...
                      JSON.stringify({
                        ...
                        item: {
                          ...
                          output: JSON.stringify({
                            success: false,
                            error:
                              'Could not generate evolution right now. Keep the conversation going on your own.',
                          }),
                        },
                      })
                    )
                ...
                dc.send(
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        success: false,
                        error: 'Pass the name you heard from the child.',
                      }),
                    },
                  })
                )
                      ...
                      JSON.stringify({
                        ...
                        item: {
                          ...
                          output: JSON.stringify({
                            success: true,
                            message:
                              "Couldn't load their profile, but that's okay — continue the conversation naturally.",
                          }),
                        },
                      })
                    )
                    ...
                    JSON.stringify({
                      ...
                      item: {
                        ...
                        output: JSON.stringify({
                          success: true,
                          message:
                            "Couldn't load their profile right now. Continue the conversation naturally.",
                        }),
                      },
                    })
                  )
            ...
            if (msg.name === 'indicate') {
              try {
                ...
                if (
                  ...
                  isFinite(Number(args.range.from)) &&
                  isFinite(Number(args.range.to))
                ) {
            ...
            if (msg.name === 'start_game') {
              try {
                ...
                if (!game) {
                  throw new Error(
                    `Unknown game: ${gameId}. Valid games: ${[...GAME_MAP.keys()].join(', ')}`
                  )
                }
                if (activeGameIdRef.current) {
                  dc.send(
                ...
                dc.send(
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        success: false,
                        error: err instanceof Error ? err.message : 'Invalid game parameters',
                      }),
                    },
                  })
                )
              ...
              dc.send(
                JSON.stringify({
                  ...
                  item: {
                    ...
                    output: JSON.stringify({
                      success: true,
                      message: `${game?.name ?? gameId} game ended.`,
                    }),
                  },
                })
              )
                ...
                dc.send(
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        success: false,
                        error: 'Invalid parameters for set_number_line_style',
                      }),
                    },
                  })
                )
        ...
        // Go active
        console.log(
          '[conference] call active — number: %d, voice: %s',
          number,
          getVoiceForNumber(number)
        )
        setState('active')
        ...
        timerRef.current = setInterval(() => {
          ...
          if (isExplorationActiveRef?.current && narrationPlayingRef.current) {
            deadlineRef.current = Math.max(
              deadlineRef.current,
              Date.now() + WARNING_BEFORE_END_MS + 5000
            )
            warningSentRef.current = false
          }
          ...
          if (activeGameIdRef.current && !extensionUsedRef.current) {
            ...
            if (gameRemaining < WARNING_BEFORE_END_MS) {
              ...
              warningSentRef.current = false
              console.log(
                '[timer] auto-extended %dms — game "%s" is active',
                EXTENSION_MS,
                activeGameIdRef.current
              )
            }
        ...
        setError(
          err instanceof Error
            ? err.message
            : `${number} couldn't pick up the phone. Try calling again!`
        )
  ...
  const sendSystemMessage = useCallback((text: string, promptResponse = false) => {
    ...
    console.log(
      '[exploration] sendSystemMessage — promptResponse:',
      promptResponse,
      'text:',
      text.slice(0, 80) + '...'
    )
