...
export const guessMyNumberGame: GameDefinition = {
  ...
  agentRules:
    ...
    '3) CRITICAL — after EVERY answer, you MUST call indicate with a range ({ from, to }) showing the remaining candidate range AND call look_at to pan/zoom the number line to that range. The child should SEE the highlighted band shrinking with each guess. This is the whole point — make the binary search VISIBLE. ' +
  ...
  onStart(params: Record<string, unknown>): GameStartResult {
    ...
    return {
      agentMessage:
        ...
        `to highlight the remaining candidate band, and call look_at to pan there. ` +
        `The child must SEE the range shrinking each time — that's the whole visual payoff. ` +
