...
export const guessMyNumberGame: GameDefinition = {
  ...
  agentRules:
    ...
    '3) SHOW the search on the number line! After each answer, call look_at to zoom into the remaining range, and use indicate with the range to highlight where the number must be. The child should SEE the range shrinking. ' +
  ...
  onStart(params: Record<string, unknown>): GameStartResult {
    ...
    return {
      agentMessage:
        ...
        `Use look_at and indicate to SHOW the narrowing range on the number line. ` +
