...
function getZoomKeyframes(
  ...
  if (constantId === 'sqrt2') {
    ...
    return [
      ...
      { progress: 0.895, center: 1.414215, pixelsPerUnit: ppuForRange(cssWidth, 1.41421, 1.41422) },
      {
        progress: 0.91,
        center: 1.4142135,
        pixelsPerUnit: ppuForRange(cssWidth, 1.414213, 1.414214),
      },
  ...
  if (constantId === 'sqrt3') {
    ...
    return [
      ...
      { progress: 0.88, center: 1.73205, pixelsPerUnit: ppuForRange(cssWidth, 1.732, 1.7321) },
      { progress: 0.895, center: 1.732052, pixelsPerUnit: ppuForRange(cssWidth, 1.73205, 1.73206) },
      {
        progress: 0.91,
        center: 1.7320508,
        pixelsPerUnit: ppuForRange(cssWidth, 1.73205, 1.732051),
      },
  ...
  if (constantId === 'pi') {
    ...
    return [
      ...
      { progress: 0.99, center: 3.141595, pixelsPerUnit: ppuForRange(cssWidth, 3.14159, 3.1416) },
      {
        progress: 1.0,
        center: 3.1415925,
        pixelsPerUnit: ppuForRange(cssWidth, 3.141592, 3.141593),
      },
    ]
  }
  ...
  if (constantId === 'ln2') {
    ...
    return [
      ...
      { progress: 0.9, center: 0.55, pixelsPerUnit: ppuForRange(cssWidth, 0.15, 1.05) },
      { progress: 1.0, center: 0.55, pixelsPerUnit: ppuForRange(cssWidth, 0.15, 1.05) },
    ]
  }
  ...
  if (constantId === 'feigenbaum') {
    ...
    return [
      ...
      { progress: 0.14, center: 2.9, pixelsPerUnit: ppuForRange(cssWidth, 2.5, 3.3) },
      // Seg 3: Approaching split point
      { progress: 0.2, center: 3.1, pixelsPerUnit: ppuForRange(cssWidth, 2.7, 3.5) },
      // Seg 3: First split at r=3.2
      { progress: 0.3, center: 3.1, pixelsPerUnit: ppuForRange(cssWidth, 2.8, 3.5) },
      // Seg 5: Moving to r=3.5
      { progress: 0.37, center: 3.3, pixelsPerUnit: ppuForRange(cssWidth, 2.9, 3.7) },
      // Seg 6: Cascade region
      { progress: 0.45, center: 3.45, pixelsPerUnit: ppuForRange(cssWidth, 3.0, 3.6) },
      // Seg 7: Zoom out for full diagram
      { progress: 0.56, center: 3.1, pixelsPerUnit: ppuForRange(cssWidth, 2.4, 3.7) },
      // Seg 7: Hold for diagram
      { progress: 0.64, center: 3.1, pixelsPerUnit: ppuForRange(cssWidth, 2.4, 3.7) },
      // Seg 8: Zoom in for gap bars
      { progress: 0.7, center: 3.25, pixelsPerUnit: ppuForRange(cssWidth, 2.9, 3.6) },
      // Seg 9: Hold on gaps region for tiling visual
      { progress: 0.82, center: 3.25, pixelsPerUnit: ppuForRange(cssWidth, 2.9, 3.6) },
      // Seg 10a: Zoom into the right end of the fractal — cascade region
      { progress: 0.85, center: 3.55, pixelsPerUnit: ppuForRange(cssWidth, 3.44, 3.58) },
      ...
      { progress: 0.94, center: 4.669, pixelsPerUnit: ppuForRange(cssWidth, 4.0, 5.3) },
      { progress: 1.0, center: 4.669, pixelsPerUnit: ppuForRange(cssWidth, 4.0, 5.3) },
    ]
  }
  ...
  if (constantId === 'ramanujan') {
    ...
    return [
      ...
      { progress: 0.1, center: 2, pixelsPerUnit: ppuForRange(cssWidth, 0, 4) },
      // Act 3: knob/curve — widen to see s=2,3,4 points [−0.5,6.5]
      { progress: 0.26, center: 3, pixelsPerUnit: ppuForRange(cssWidth, -0.5, 6.5) },
      // Act 4: pole — hold view for curve + approaching wall [−1,6]
      { progress: 0.42, center: 2.5, pixelsPerUnit: ppuForRange(cssWidth, -1, 6) },
      // Act 5: bridge — failed attempts + smooth continuation [−3,5]
      { progress: 0.52, center: 1, pixelsPerUnit: ppuForRange(cssWidth, -3, 5) },
      // Act 5→6 transition: crossing zero, flipper concept [−3,3]
      { progress: 0.65, center: 0, pixelsPerUnit: ppuForRange(cssWidth, -3, 3) },
      // Act 6: flipper + connect — see both sides [−2,2]
      { progress: 0.7, center: 0, pixelsPerUnit: ppuForRange(cssWidth, -2, 2) },
      // Act 7: volcano — tight on s=−1 [−2,1]
      { progress: 0.81, center: -0.5, pixelsPerUnit: ppuForRange(cssWidth, -2, 1) },
      // Act 7: trust — zoom OUT to see convergent points at s=2,3,4 [−1.5,5]
      { progress: 0.85, center: 1.75, pixelsPerUnit: ppuForRange(cssWidth, -1.5, 5) },
      ...
      { progress: 1.0, center: -0.5, pixelsPerUnit: ppuForRange(cssWidth, -2, 1) },
    ]
  }
...
export function useConstantDemo(
  ...
  const tickDemo = useCallback(() => {
    ...
    if (ds.phase === 'animating') {
      ...
      if (ds.constantId) {
        const kfs = getZoomKeyframes(ds.constantId, cssWidthRef.current, cssHeightRef.current)
        if (kfs) {
  ...
  const setRevealProgress = useCallback(
    (value: number) => {
      const ds = demoStateRef.current
      if (ds.phase === 'idle') return
