...
export const ASSEMBLY = {
  ...
  apartSpeed: 1.6,
  /** sliding_dovetail: how far along the seam the module lines up, × depth
   *  (in front for groove-carriers, behind for rail-carriers) */
  slideBehindFactor: 1,
 ...
 *     still `depth` along the seam from the seat (sliding; which way depends on
 *     the face it presents — see below) or `liftFactor·depth` above it
...
export function modulePose(
  ...
  if (joint === 'sliding_dovetail') {
    // The male rail (scad slide_tabs, on every module's RIGHT face) runs
    // nose-forward: narrow at the blind front stop, deep anchor flush to the
    // rear face. The female groove (slide_pockets, LEFT face) opens at its
    // rear mouth. The nose always enters the mouth first and the anchor
    // seats LAST, so which way a module slides depends on which face it
    // presents to the chain: right of the anchor it carries the GROOVE —
    // its mouth starts at the neighbour's nose, so it stages in FRONT (−Y)
    // and slides rearward; left of the anchor it carries the RAIL — its
    // nose starts at the neighbour's mouth, so it stages BEHIND (+Y) and
    // slides forward. One uniform direction rams the deep anchor through
    // the narrow stop on one side (user review 2026-09-10 / 09-14). Apart
    // is this exact path in reverse.
    const dir = i > anchor ? -1 : 1 // sign of the staging offset along Y
    const stage = dir * dims.depth * ASSEMBLY.slideBehindFactor
    if (u <= A) {
      // phase A: line up on the joint axis (x closes on the anchor, Y opens)
      const a = easeInOut(A > 0 ? u / A : 1)
      return pose(home + (seatX - home) * a, stage * a, 0)
    }
    // phase B: x is home; run the one legal axis nose-through-mouth, the
    // detent flicking past the seat on the far side from the stage
    const b = (u - A) / (1 - A)
    return pose(seatX, seatWithClick(b, stage, -dir * ASSEMBLY.clickOvershootMm), 0)
  }
