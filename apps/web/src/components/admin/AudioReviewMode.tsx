...
export function AudioReviewMode({
  ...
  const [reviewSubset, setReviewSubset] = useState<AudioClipEntry[] | null>(null)
  ...
  const progressPercent =
    clips.length > 0 ? Math.round(((currentIndex + 1) / clips.length) * 100) : 0
      ...
      >
        ...
        {flaggedClips.length === 0 ? (
          ...
          <>
            ...
            >
              {flaggedClips.length} clip{flaggedClips.length !== 1 ? 's' : ''} flagged for
              regeneration:
            </p>
              ...
              {flaggedClips.map((clip) => (
                ...
                >
                  ...
                  >
                    ...
                    <span className={css({ color: '#c9d1d9' })}>{clip.text}</span>
                  </div>
    ...
    >
      ...
      >
        ...
        <button
          ...
          className={`${buttonBase} ${css({
            backgroundColor: currentClip && flagged.has(currentClip.id) ? '#f85149' : '#3d1f28',
            color: currentClip && flagged.has(currentClip.id) ? '#fff' : '#f85149',
      ...
      >
        ...
        {isAutoAdvance && <span className={css({ color: '#3fb950', fontSize: '12px' })}>ON</span>}
      </div>
    </section>
  )
}
