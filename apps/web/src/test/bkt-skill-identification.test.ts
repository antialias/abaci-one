...
function generateSyntheticResults(
  ...
  if (skillPool.length < minSkillsPerProblem) {
    throw new Error(`Need at least ${minSkillsPerProblem} skills in pool, got ${skillPool.length}`)
  }
  ...
  for (let i = 0; i < problemCount; i++) {
    ...
    const numSkills =
      minSkillsPerProblem +
      Math.floor(Math.random() * (maxSkillsPerProblem - minSkillsPerProblem + 1))

    // Select random skills for this problem
    const shuffled = [...skillPool].sort(() => Math.random() - 0.5)
    const problemSkills = shuffled.slice(0, Math.min(numSkills, skillPool.length))
    ...
    if (includeWeakSkill && !problemSkills.includes(weakSkill)) {
      ...
      problemSkills[Math.floor(Math.random() * problemSkills.length)] = weakSkill
    }
...
function createWeakSkillScenario(
  ...
  const skillPool = [weakSkillId, ...otherSkills.filter((s) => s !== weakSkillId)]
...
describe('BKT Skill Identification', () => {
  ...
  describe('Systematic Skill Testing', () => {
    ...
    const testCases: Array<{
      ...
      },
      {
        ...
        companions: ['basic.directAddition', 'basic.heavenBead', 'fiveComplements.4=5-1'],
      },
    ]
  ...
  describe('Conjunctive Model Validation', () => {
    it('should correctly distribute blame across multi-skill problems', () => {
      ...
      if (resultA && resultB && resultA.confidence > 0.2 && resultB.confidence > 0.2) {
        const avgWeakPKnown = (resultA.pKnown + resultB.pKnown) / 2
        const avgStrongPKnown = ((resultC?.pKnown ?? 0) + (resultD?.pKnown ?? 0)) / 2
