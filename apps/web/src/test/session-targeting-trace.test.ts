...
import { starkContrastProfile, STARK_WEAK_SKILLS } from './journey-simulator/profiles'
...
describe('Session Targeting Trace', () => {
  ...
  it('should trace targetSkills through session generation', async () => {
    ...
    try {
      ...
      console.log(`Slots with targeting: ${slotsWithTargeting}/${session2TotalSlots}`)

      // Now run CLASSIC mode with same seed for comparison
      console.log('\n--- CLASSIC MODE COMPARISON ---')
      const { playerId: classicPlayerId } = await createTestStudent(ephemeralDb.db, 'classic-trace')
      await setPracticingSkills(classicPlayerId, skills)

      const classicRng = new SeededRandom(seed)
      const classicStudent = new SimulatedStudent(starkContrastProfile, classicRng)
      ...
      console.log(`Classic slots with targeting:  ${classicSlotsWithTargeting}`)

      const adaptiveRate = session2FiveComplementCount / session2TotalSlots
      const classicRate = classicSession2FiveComplementCount / classicSession2TotalSlots
      ...
      if (improvement > 0.1) {
        ...
        console.log('  This suggests the problem generator is not responding to targetSkills.')
      } else {
